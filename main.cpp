...
vector<vector<float>> Linear_layer(vector<vector<float>> input){
    vector<vector<float>> output;
    vector<vector<float>> weight_matrix = {{10.2, -0.9,2.2}, 
                                           { 8.1, 3.3,  -1.2},
                                           { 7.2, -8.9, -3.8},
                                           {-3.5, 7.1,  -10},
                                           {-2.0, 4.2,  -6.6}};
    ...
    for(int i = 0; i < size(input) ; i++){
        for(int j = 0 ; j < size(input[0]) ; j++){
            for (int m = 0; m < size(weight_matrix[0]) ; m++){
                output[i][m] += input[i][j] * weight_matrix[j][i];
            }
        }
    }
...
int main (){
    ...
    output1 = Linear_layer(input_matrix);
